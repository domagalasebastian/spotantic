...
@pytest.mark.asyncio
async def test_get_user_profile_builds_request_and_returns_parsed_data():
    ...
    with (
        mock.patch.object(GetUserProfileRequest, "build", return_value=request_obj) as build_mock,
        mock.patch.object(UserModel, "model_validate", return_value=user_model) as validate_mock,
    ):
        result = await get_user_profile(client, user_id="user123")

        build_mock.assert_called_once_with(user_id="user123")
