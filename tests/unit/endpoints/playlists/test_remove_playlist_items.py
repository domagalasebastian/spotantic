...
@pytest.mark.asyncio
async def test_remove_playlist_items_builds_request_and_returns_model():
    ...
    with mock.patch.object(RemovePlaylistItemsRequest, "build", return_value=request_obj) as build_mock:
        with mock.patch(
            "spotantic.models.playlists.responses.PlaylistSnapshotResponseModel.model_validate", return_value=fake_model
        ) as validate_mock:
            result = await remove_playlist_items(client, playlist_id="p1", uris=["spotify:track:1"], snapshot_id="s1")

            build_mock.assert_called_once_with(playlist_id="p1", uris=["spotify:track:1"], snapshot_id="s1")
