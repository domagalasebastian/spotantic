...
from spotantic.endpoints.playlists import get_user_playlists
from spotantic.models.playlists.requests import GetUserPlaylistsRequest


@pytest.mark.asyncio
async def test_get_user_playlists_builds_request_and_returns_model():
    ...
    with mock.patch.object(GetUserPlaylistsRequest, "build", return_value=request_obj) as build_mock:
        with mock.patch(
            "spotantic.models.spotify.PagedResultModel.model_validate", return_value=fake_model
        ) as validate_mock:
            result = await get_user_playlists(client, user_id="u1", limit=10, offset=5)

            build_mock.assert_called_once_with(user_id="u1", limit=10, offset=5)
