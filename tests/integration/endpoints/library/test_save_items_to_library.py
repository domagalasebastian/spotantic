...
async def spotify_item_uri(client, example_spotify_uri):
    ...
    uris_to_remove = [uri for uri, is_saved in data.data.items() if not is_saved]

    if uris_to_remove:
        await remove_items_from_library(client, uris=uris_to_remove)


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_save_items_to_library(client, spotify_item_uri):
    artist_uri, album_uri = spotify_item_uri

    result = await save_items_to_library(client, uris=[artist_uri, album_uri])

    assert isinstance(result.request, SaveItemsToLibraryRequest)
