...
async def album_id(client, example_spotify_item_id):
    ...
    if not is_present_in_library:
        await remove_user_saved_albums(client, album_ids=[album_id])


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_save_albums_for_current_user(client, album_id):
    result = await save_albums_for_current_user(client, album_ids=[album_id])

    assert isinstance(result.request, SaveAlbumsForCurrentUserRequest)
