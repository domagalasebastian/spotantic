...
async def episode_id(client, example_spotify_item_id):
    ...
    if not is_present_in_library:
        await remove_user_saved_episodes(client, episode_ids=[episode_id])


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_save_episodes_for_current_user(client, episode_id):
    result = await save_episodes_for_current_user(client, episode_ids=[episode_id])

    assert isinstance(result.request, SaveEpisodesForCurrentUserRequest)
