...
async def track_id(client, example_spotify_item_id):
    ...
    if not is_present_in_library:
        await remove_user_saved_tracks(client, track_ids=[track_id])


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_save_tracks_for_current_user(client, track_id):
    result = await save_tracks_for_current_user(client, track_ids=[track_id])

    assert isinstance(result.request, SaveTracksForCurrentUserRequest)
