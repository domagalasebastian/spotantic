...
async def playlist_id(client, example_spotify_item_id):
    ...
    if not is_present_in_library:
        await unfollow_playlist(client, playlist_id=item_id)


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_follow_playlist(client, playlist_id):
    result = await follow_playlist(client, playlist_id=playlist_id)

    assert isinstance(result.request, FollowPlaylistRequest)
