...
async def item_data(request, client, example_spotify_item_id):
    ...
    if not is_present_in_library:
        await unfollow_artists_or_users(client, item_ids=[item_id], item_type=item_type)
...
async def test_follow_artists_or_users(client, item_data):
    ...
    assert isinstance(result.request, FollowArtistsOrUsersRequest)
