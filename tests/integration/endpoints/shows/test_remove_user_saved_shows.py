...
async def show_id(client, example_spotify_item_id):
    ...
    if is_present_in_library:
        await save_shows_for_current_user(client, show_ids=[show_id])


@pytest.mark.asyncio
@pytest.mark.mutation
@pytest.mark.user_library
async def test_remove_user_saved_shows(client, show_id):
    result = await remove_user_saved_shows(client, show_ids=[show_id])

    assert isinstance(result.request, RemoveUserSavedShowsRequest)
